...
function InfoCard({ officeID = 'office_00', item }) {
  ...
  const handleSubmit = evt => {
    ...
    set(entryRef,
      formState).finally(res => setEditing(false))
  }
  ...
  return (
    <div className='info-card'>
      ...
      {editing ? null : <div className="controls"><button onClick={() => setEditing(true)}>Edit</button><button onClick={() => handleRemove()}>Remove</button></div>}
    </div>
  );
}
